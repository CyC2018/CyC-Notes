...
class Solution {
    public boolean isBalanced(TreeNode root) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (hash.containsKey(cur.left) && hash.containsKey(cur.right)) {
                if (Math.abs(hash.get(cur.left) - hash.get(cur.right)) > 1) {
                    return false;
                } else {
                    hash.put(cur, Math.max(hash.get(cur.left), hash.get(cur.right)) + 1);
                }
            } else {
                stack.add(cur);
